...
// Route to create a new Fedex Order
router.post(
  "/createfedexorder/international",
  authMiddleware,
  async (req, res) => {
    let {
      senderName,
    ...
    try {
      ...
      const newOrder = new FedexOrderInternational({
        note: specialNotes,
        price,
...
router.get("/getAllOrdersInternational", authMiddleware, async (req, res) => {
  try {
    const orders = await FedexOrderInternational.find({ userId: req.userId }) // Filter by userId
      .populate(
...
router.put(
  "/updateOrderStatusInternational/:id",
  authMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const order = await FedexOrderInternational.findOne({
        _id: req.params.id,
        userId: req.userId,
      })
        .populate(
...
router.post(
  "/duplicateOrderInternational/:id",
  authMiddleware,
  async (req, res) => {
    try {
      const existingOrder = await FedexOrderInternational.findOne({
      ...
      const newOrder = new FedexOrderInternational({
        ...
        pickAddress: duplicatePickAddress
          ? duplicatePickAddress._id
          : undefined,
...
router.get(
  "/getOrderByIdInternational/:orderId",
  authMiddleware,
  async (req, res) => {
