    ...
    const fetchOrderDetails = async () => {
      try {
        ...
        if (orderId.startsWith("SHFEDO")) {
          response = await axios.get(`${baseUrl}/api/getOrderByIddomestic/${orderId}`,{
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
        } else {
          response = await axios.get(`${baseUrl}/api/getOrderByIdInternational/${orderId}`,{
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
...
export default FedexOrderDetails;
