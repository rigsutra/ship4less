...
function FedexOrderListDomestic() {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/api/getAllOrdersdomestic`,
          {
            headers: {
        ...
        setLoading(false);
        console.log("Fedex Domestic Orders:", response.data);
      } catch (error) {
        toast({
  ...
  const handleDuplicateOrder = async (orderId) => {
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/duplicateOrderdomestic/${orderId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      console.error("Error duplicating order frontend:", error);
      toast({
  ...
  return (
    <div className="">
      <TopBar title={"FedEx Domestic"} />
      <div className="px-8 py-2 ">
        <HStack justify="space-between" pt={10} pb={5}>
          <Text fontSize="2xl">FedEx Domestic Orders</Text>
          <Button
            colorScheme="blue"
            onClick={() => handleCreateOrder("domestic")}
          >
            + Create Order
          </Button>
        </HStack>
...
export default FedexOrderListDomestic;
