...
const QuickOrderForm = () => {
  ...
  const handleQuickOrderSubmit = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(`${baseUrl}/api/orders/quick`, quickOrderData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
const AddressSection = ({ title, selectedAddress, setSelectedAddress, addresses }) => (
  <VStack
    spacing={4}
    p={4}
    bg="gray.800"
...
const NormalOrderForm = () => {
  ...
  const handleNormalOrderSubmit = async () => {
    ...
    try {
      setLoading(true);
      await axios.post(`${baseUrl}/api/orders/normal`, normalOrderData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
        ...
        <SimpleGrid columns={[1, null, 2]} spacing={6} mb={4}>
          <VStack spacing={4}>
            ...
            <Select
              ...
              placeholder="Select type"
            >
              <option value="UPS Ground OZ">UPS Ground OZ</option>
              <option value="UPS Ground lb">UPS Ground lb</option>
              <option value="UPS Priority">UPS Priority</option>
              <option value="UPS Express">UPS Express</option>
              <option value="UPS Priority v2">UPS Priority v2</option>
            </Select>
          </VStack>
